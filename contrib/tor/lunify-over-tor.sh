...
if test -z "$lunifyd"
then
  echo "lunifyd not found"
  exit 1
fi
echo "Found: $lunifyd"

TORDIR="$DIR/lunify-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting lunifyd"
  tail -n 400 "$HOME/.Lunify/Lunify.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Lunify\ \'" | tail -n 20
  exit 1
fi
