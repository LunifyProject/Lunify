# Anonymity Networks with Lunify

Currently only Tor and I2P have been integrated into Lunify. The usage of
...
is made or until `lunifyd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `lunify-wallet-cli` and
`lunify-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `lunifyd` the ip/port of a socks proxy provided by a
...
which tells `lunifyd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `lunifyd` the inbound address, network
...
which tells `lunifyd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `lunifyd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `lunifyd` localhost port 30000.
...
`lunifyd` RPC port - which is independent from the configuration for incoming
P2P anonymity connections. The anonymity network (Tor/i2p) is
[configured in the same manner](#configuration), except the localhost port
must be the RPC port (typically 25252 for mainnet) instead of the p2p port:

```
HiddenServiceDir /var/lib/tor/data/lunify
HiddenServicePort 25252 127.0.0.1:25252
```
...
This will store key information in `/var/lib/tor/data/lunify` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/lunify/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the lunify network should have
similar timestamps.
...
### Intermittent lunify Syncing

If a user only runs `lunifyd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `lunifyd` as often as possible to conceal when transactions are being sent.
...
`lunifyd` currently selects two outgoing connections every 5 minutes for
...
`lunifyd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `lunifyd` should likely disconnect
