# The Current/Future Status of ZMQ in lunify

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. lunify
...
matching supported by both lunify and ZMQ. The `format`, `context` and `event`
...
The scallunifya daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The lunify daemon also ensures that
...
The lunify daemon will send a `txpool_add` pub exactly once for each
