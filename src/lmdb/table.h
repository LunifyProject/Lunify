...
//! Helper for grouping typical LMDB DBI options when key and value are fixed
//! types.
template <typename K, typename V> struct basic_table : table {
  using key_type = K;
  using value_type = V;
  ...
  constexpr explicit basic_table(const char *name, unsigned flags = 0,
                                 MDB_cmp_func value_cmp = nullptr) noexcept
      : table{name, compute_flags(flags), &lmdb::less<lmdb::native_type<K>>,
              value_cmp} {}
  ...
  /*!
      ...
      \note If using `F` and `offset` to retrieve a specific field, use
          `lunify_FIELD` macro in `src/lmdb/util.h` which calculates the
  ...
  */
  ...
  static expect<F> get_value(MDB_val value) noexcept {
    static_assert(std::is_same<U, V>(), "bad lunify_FIELD?");
    static_assert(std::is_pod<F>(), "F must be POD");
    static_assert(sizeof(F) + offset <= sizeof(U),
    ...
    std::memcpy(std::addressof(out),
                static_cast<char *>(value.mv_data) + offset, sizeof(out));
    return out;
  }
  ...
  */
  template <typename D>
  expect<key_stream<K, V, D>> static get_key_stream(
      std::unique_ptr<MDB_cursor, D> cur) noexcept {
    lunify_PRECOND(cur != nullptr);
