...
#define lunify_DEFAULT_LOG_CATEGORY "daemon.rpc.bootstrap_daemon"

namespace cryptonote {

bootstrap_daemon::bootstrap_daemon(
    std::function<std::map<std::string, bool>()> get_public_nodes,
    bool rpc_payment_enabled, const std::string &proxy)
    : m_selector(
          new bootstrap_node::selector_auto(std::move(get_public_nodes))),
...
bootstrap_daemon::bootstrap_daemon(
    ...
    bool rpc_payment_enabled, const std::string &proxy)
    : m_selector(nullptr), m_rpc_payment_enabled(rpc_payment_enabled) {
  set_proxy(proxy);
...
bool bootstrap_daemon::handle_result(bool success, const std::string &status) {
  const bool failed = !success || (!m_rpc_payment_enabled &&
                                   status == CORE_RPC_STATUS_PAYMENT_REQUIRED);
...
bool bootstrap_daemon::set_server(
    const std::string &address,
